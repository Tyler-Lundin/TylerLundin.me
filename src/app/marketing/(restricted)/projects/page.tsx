...
function StatusBadge({ value }: { value: string }) {
  ...
  return <span className={`inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium border capitalize ${style}`}>{value.replace('_', ' ')}</span>
}
        ...
        <div className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          ...
          <div className="flex items-center gap-2">
            <div className="relative hidden sm:block">
              ...
              <input disabled placeholder="Search projects..." className="h-10 w-64 rounded-lg border border-neutral-200 bg-white pl-10 pr-4 text-sm outline-none ring-offset-2 focus:ring-2 focus:ring-neutral-900 disabled:opacity-60 dark:border-neutral-800 dark:bg-neutral-900 dark:focus:ring-white" />
            </div>
                  ...
                  <tr key={p.id} className="group transition-colors hover:bg-neutral-50 dark:hover:bg-neutral-800/50">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <Link className="font-medium text-neutral-900 hover:underline dark:text-white" href={`/marketing/projects/${p.slug}`}>
                          {p.title}
                        </Link>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <Link className="text-neutral-600 hover:text-neutral-900 hover:underline dark:text-neutral-300 dark:hover:text-white" href={`/marketing/clients/${slugify(p.client?.name || '')}`}>
                        {p.client?.name || 'Unknown'}
                      </Link>
                    </td>
                    <td className="px-6 py-4"><StatusBadge value={p.status} /></td>
                    <td className="px-6 py-4"><PriorityPill value={p.priority} /></td>
